...
usage() {
   ...
   echo "  -u <version>    ubuntu version to check in artifactory, eg: 1404, 1804, 2204, 2404, 2604, default: $DEFAULT_UBUNTU_VERSION" 1>&2
...
if [ "$LATEST_SHA" == "1" ]; then
   for SHA in $SHAS
   do
      LINK="https://pure-artifactory.dev.purestorage.com/artifactory/iridium-artifacts/build/$SHA/ubuntu${UBUNTU_VERSION}/sim-iros.img.gz"
...
do
   ...
   IROS_LINK="https://pure-artifactory.dev.purestorage.com/artifactory/iridium-artifacts/build/$SHA/ubuntu${UBUNTU_VERSION}/sim-iros.img.gz"
